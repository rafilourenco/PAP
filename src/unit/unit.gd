...
signal moved_to_tile(pos: Vector2)
...
var health: int = 3: set = _set_health
...
func move_along_path(path: Array) -> void:
	...
	for pcell in path:
		...
		move_tween.tween_callback(func(): _emit_moved_to_tile(Navigation.cell_to_world(pcell, true)))
		if pcell.x < cell.x:
			sprite.flip_h = true
		elif pcell.x > cell.x:
			sprite.flip_h = false
...
func _emit_moved_to_tile(pos: Vector2) -> void:
	moved_to_tile.emit(pos)
